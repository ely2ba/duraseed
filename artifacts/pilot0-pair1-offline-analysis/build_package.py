...
def cell(kind, text, number):
    row = {"cell_type": kind, "id": f"pair1-{number:02d}",
...
def main():
    intro = """# Pair-1 offline analysis package
...
package = repo / "artifacts/pilot0-pair1-offline-analysis"
...
print("Pair-1 artifact paths only; no remote clients or new samples.")
'''
    ...
    def execute(row):
        ...
        with redirect_stdout(stdout), redirect_stderr(stderr):
            exec(compile("".join(row["source"]), "pair1-notebook-cell", "exec"), namespace)
        execution_count += 1
    ...
    notes_snapshot = notes.replace("../artifacts/pilot0-pair1-offline-analysis/README.md", "README.md")
...
'''
    ...
    (PACKAGE / "pair1-offline-analysis.ipynb").write_text(json.dumps(notebook, indent=2, allow_nan=False)+"\n")
    navigation = """

"""Local-only presentation of the completed pair-1 validation artifacts."""
...
def make_figure(path, panels, title):
    ...
    parts = [
        ...
        '<text x="55" y="61" class="small">Pair 1 · seed 11 · x-axis: log(1 + update); AUC calculations use the linear update grid</text>',
    ]
...
matching = read(ROOT / "seed-11/matching.json")
extracted = {}
for method in METHODS:
    base = ROOT / "seed-11" / method
    ...
    reproduced = canonical_json_value(summarize_selected_method(
        seed=11, method=method, stage_a_selected=before,

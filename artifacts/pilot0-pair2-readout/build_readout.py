...
payload = {"run_id": ROOT.name, "seed": 29, "stage_b_updates": GRID,
           ...
           "F3_source": f"{ROOT_REF}/seed-29/pre-b-profiles.json"}
...
lines = ["# Pair-2 scientific readout", "", f"Run: `{ROOT.name}`. Seed: 29. Local artifacts only.", "",
    ...
    f"Raw source references ([data download and layout](../../docs/pilot0-data.md)): terminal result (`{ROOT_REF}/result.json`), matching record (`{ROOT_REF}/seed-29/matching.json`), [exact readout data](readout.json), [both F3 profiles](F3.md).", "",

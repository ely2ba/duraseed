"""Generate two descriptive README SVGs from the existing pair-2 count readout."""
...
def begin(title, description):
    return [
        ...
        text(40, 66, "Pilot pair 1 · seed 29 · selected Stage-A checkpoints: B-S 40 / B-G 20", 15),
    ]
...
def retention():
    ...
    finish(svg, "pilot-pair2-retention.svg")
...
def learning():
    ...
    finish(svg, "pilot-pair2-learning.svg")


if __name__ == "__main__":
    assert DATA["seed"] == 11
    assert DATA["matching"]["B-S"]["step"] == 40 and DATA["matching"]["B-G"]["step"] == 20
    retention()
    learning()

...
def start_calibration_run(inputs: Any, root: Path) -> RunRecord:
    ...
    expected = RunRecord(
        ...
        authorized_cost_usd=inputs.stage_a_ledger.authorized_usd,
        reserved_cost_usd=inputs.stage_a_ledger.authorized_usd,
...
def finish_calibration_run(
    ...
    observed = inputs.stage_a_ledger.observed
    cost = max(
        record.cost_usd,
    ...
    checkpoint_rows = []
    for path in (root / "stage-a-arms/checkpoint-ttl-audit.json",):
        if path.exists():
            value = _json(path)
            rows = value.get("rows") if isinstance(value, dict) else None

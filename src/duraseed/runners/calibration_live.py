...
class CalibrationLiveInputs:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            self.teacher_ledger.authorized_usd + self.stage_a_ledger.authorized_usd
            > 300
        ):
            raise ValueError("calibration action allocations exceed $300")
...
async def run_live_calibration(inputs: CalibrationLiveInputs) -> dict[str, Any]:
    ...
    from duraseed.runners.teacher_dose_live import collect_teacher_dose
    ...
    try:
        if "teacher-dose" not in artifacts:
            evidence = await collect_teacher_dose(
                inputs,
                root / "teacher-dose-arms",
            ...
            recipe = select_teacher_recipe(inputs.config, evidence)
        ...
        if "teacher-allocation" not in artifacts:
            sources = replace(inputs.teacher_sources, selected_dose=selected_dose)

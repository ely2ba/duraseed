...
def font_style():
    ...
    for weight, name in ((400, "Regular"), (600, "Semibold")):
        data = b64encode((OUTPUT / f"fonts/SourceSans3-{name}.woff2").read_bytes()).decode()
        rules.append('@font-face{font-family:"Source Sans 3";font-style:normal;'
                     f'font-weight:{weight};src:url(data:font/woff2;base64,{data}) format("woff2")}}')
    rules.append('text{font-family:"Source Sans 3",sans-serif;'
...
def coverage():
    ...
    svg = start(608, "Arithmetic problem coverage",
    ...
    text(svg, 60, 87, "256 shared arithmetic problems · 16 attempts each · replay block 11", 17, MUTED)
    for arm, x, title in (("R-S", 75, "Solver traces"), ("R-P", 580, "Policy traces")):
        ...
        text(svg, x + 397, 178, f'{100 * panel["exact_success_rate"]:.2f}%', 25,
        ...
        for k, count in enumerate(histogram):
            ...
            if k in (0, 16):
                text(svg, cx, bottom - height - 9, count, 16, weight=600, anchor="middle")
        text(svg, x + width / 34, 525, "None", 13, MUTED, anchor="middle")
        text(svg, x + width - width / 34, 525, "All 16", 13, MUTED, anchor="middle")
        text(svg, x + width / 2, 554, "Correct attempts per problem (out of 16)", 16,
             anchor="middle")
    text(svg, 60, 589, "Each bar counts problems with exactly that many correct attempts. Both panels use the same scale.", 15, MUTED)
...
def trajectories():
    ...
    svg = start(568, "Retention and subsequent learning",
    ...
    for arm in ARMS:
        ...
        if arm == "R-P":
            ...
            text(svg, 267, 258, f'{100 * curve["raw"][curve["grid"].index(10)]:.2f}%', 15, MUTED)
    ...
    text(svg, 60, 552, "Points show recorded checkpoints; straight segments connect the observations.", 14, MUTED)
...
def embed_pipeline_fonts():
    ...
    svg = path.read_text()
    svg = re.sub(r"\s*<!-- Embedded fonts -->.*?<!-- End embedded fonts -->", "", svg, flags=re.S)
    svg = svg.replace("</style>", f"<!-- Embedded fonts -->\n{font_style()}\n"
                      "<!-- End embedded fonts -->\n</style>", 1)
    path.write_text(svg)
